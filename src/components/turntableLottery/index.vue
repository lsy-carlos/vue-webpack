      ...
      <div class="win-prize-list">
        <div class="list-wrapper">
          ...
          <div class="list-box">
            <ul class="prize-list">
              <li
                v-for="(item, index) in prizeUsers"
                :key="index"
  ...
  methods: {
    turntableDraw(num) {
      ...
      if (userList > 10) {
        ...
        elUser.style.animation = `translatefresh ${userList +
          9}s infinite linear`; //设置动画
        let iStyle = document.styleSheets[0]; //样式表
        iStyle.insertRule(
          `@keyframes translatefresh {from { transform: translateY(300px) }to {transform: translateY(-${userList *
            30}px)}}`,
          0
        ); //插入动态动画设置
      }
      ...
      for (let i = 0; i < num; i++) {
        ...
        ctx.fillStyle = "purple"; //设置填充颜色为紫色
        ctx.font = '20px "微软雅黑"'; //设置字体 //ctx.strokeText( "left", 450, 400 );
